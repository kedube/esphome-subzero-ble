...
external_components:                          # <1>
  - source:
      ...
      ref: v3.4.0 # use latest tag, or main
...
TIP: Pin to a release tag (e.g. `ref: v3.4.0`) for stability, or use `ref: main` to track the latest changes. The minimal copy-paste examples in `wolf-minimal-fridge.yaml`, `wolf-minimal-range.yaml`, and `cove-minimal-dishwasher.yaml` in the repo root track `main` and are a known-good starting point.
...
| `hide_fridge_zone` | `false` | Set Temperature, Door (set true for wine-only fridges that have no main fridge compartment)
