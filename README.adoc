...
packages:
  - url: https://github.com/JonGilmore/esphome-subzero-ble
    ref: v0.1.2 # use latest tag, or main
    files:
...
NOTE: Use a unique `prefix` for each appliance (e.g. `szg`, `szg2`, `szg3`). See link:docs/advanced.adoc#ble-stack-tuning[BLE Stack Tuning] for `sdkconfig_options` needed when running 2+ concurrent connections.

TIP: Pin to a release tag (e.g. `ref: v0.1.2`) for stability, or use `ref: main` to track the latest changes
