...
TIP: The multi-appliance `subzero.yaml` in the repo root tracks `main`; keep appliance setup in `settings.yaml` and credentials in `secrets.yaml`. The release workflow keeps the `sw_version` substitution in `settings-example.yaml` stamped to the latest release number. Regular CI only validates and compiles the example; it does not commit version changes. If you prefer fully reproducible builds, change `ref: main` to a release tag such as `ref: v3.8.0`.
