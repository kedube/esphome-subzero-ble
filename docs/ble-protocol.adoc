...
. **Session-alive signal:** when layer 1 fires, `poll_in_flight` and `poll_no_response` are cleared. The appliance DID respond (data was flowing, we just couldn't reassemble it), so it should not count as an unanswered poll. Without this, fw 8.5 appliances whose 1500+ byte full-state response reliably hits a fragment drop would cycle in a reconnect loop forever - reconnecting doesn't help because the ACL bug persists on the new connection. Instead we stay connected and rely on push notifications for the sensors we can get.
+
