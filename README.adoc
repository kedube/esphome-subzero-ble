...
TIP: The multi-appliance `subzero-example.yaml` in the repo root tracks `main`; edit `settings-example.yaml` for appliance MACs, PINs, names, and per-appliance options. The release workflow keeps the `sw_version` substitution in that settings file stamped. If you prefer fully reproducible builds, change `ref: main` to a release tag such as `ref: v3.8.0`.
