...
* Bare-root full-state push (no marker at all) - observed 2026-07-25 on a fridge (CL4850UFDID, fw 2.27) shortly after a burst of `set` writes: the full property dump (every field, including the `notifs` array) lands directly at the JSON root with *none* of `status`/`resp`, `props`, or `msg_types` present. Same category of gap as the `msg_types:1` case above - `extract_data` didn't recognize it and logged `Parse failed or status!=0, skipping` for every occurrence. Now recognized via the presence of `appliance_serial`, which every real full dump includes regardless of wrapper shape (see `extract_data` in `subzero_protocol/protocol.cpp`).
+
...
Silent-poll accounting (poll vs push):: `poll_ok_` only flips on a successful POLL RESPONSE - the parser reports `is_poll` for `{"status":0,"resp":{...}}` messages, and pushes never count. The fw 8.5 silent-poll case keeps emitting `msg_types:1` pushes while `get_async` writes go silent server-side; treating those pushes as "evidence of life" would mask the silence and the appliance's poll-only state would freeze on its initial values with nothing in the logs to show it. With this gating, pushes still update incremental state via the dispatch bus, while `poll_miss_` increments each silent 60s window as a *diagnostic* counter (`miss=N` in the logs). Recovery is handled separately by the scheduled ~18-minute session refresh (disconnect + fast reconnect) - matching the Sub-Zero Android app's disconnect-between-polls approach on fw 8.5, just on a fixed cadence instead of a miss threshold.
...
ACL Fragment Drops (ESP32-S3) - fixed in ESP-IDF 5.5.5 / ESPHome 2026.7.1:: A bug in the ESP-IDF Bluedroid HCI layer caused ACL fragment reassembly failures during rapid bursts of indications.
This manifested as `BT_HCI: ACL packet too short` and `reassemble_and_dispatch found unfinished packet` errors.
When a fragment was dropped, the JSON reassembly buffer was left with incomplete data, and in some cases trailing non-UTF-8 bytes (stale memory) leaked into the buffer.
This only affected appliances using 40-byte indication fragments (API 5.4 / fw 8.5, e.g. SO3050PESP wall oven, 313 fridge, DEU2450R under-counter fridge, possibly others).
Appliances using 244-byte fragments (API 5.5 / fw 2.27, e.g. DF36450GSP range, DW2450 dishwasher, IW30R wine storage) were unaffected since their push notifications fit in a single indication.
+
The root cause was a bounds check in `reassemble_and_dispatch()` that required every ACL packet to be at least `HCI_ACL_PREAMBLE_SIZE + L2CAP_LENGTH_SIZE` bytes. That is only true of START fragments - short CONTINUATION fragments were rejected outright. Espressif hoisted the check into the START-only branch in https://github.com/espressif/esp-idf/commit/511e824085ed4edc845dda66abaa5f4bdf5572a4[commit `511e8240`] (https://github.com/espressif/esp-idf/issues/18414[esp-idf#18414]), released in ESP-IDF v5.5.5 and picked up by ESPHome 2026.7.1.
+
NOTE: This project previously shipped a `patch_acl_reassembly` external component that overwrote `packet_fragmenter.c` at build time. It was removed once the fix landed upstream - against ESP-IDF 5.5.5+ the frozen copy actually reverted upstream's stale-partial-packet cleanup. `subzero.yaml` now declares `esphome: min_version: 2026.7.1` instead. The buffer-handling described below is unchanged - it is defensive and useful regardless.
+
